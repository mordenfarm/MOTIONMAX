...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300 font-sans`}>
      ...
      {/* Sidebar - GitHub/Google Hybrid Style */}
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 md:w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transform transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) md:relative md:translate-x-0
      ...
      `}>
        <div className="h-full flex flex-col pt-20 md:pt-0">
          ...
          <div className="px-6 md:mt-8">
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-1.5 flex gap-1 border border-slate-200/50 dark:border-slate-700/50">
              <button 
          ...
          <nav className="flex-1 px-4 mt-8 space-y-1.5 overflow-y-auto custom-scrollbar">
            <p className="px-4 text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.25em] mb-4">Operations Hub</p>
      ...
      <main className="flex-1 flex flex-col min-h-0 bg-slate-50/50 dark:bg-slate-950">
        {/* Desktop Navbar */}
        <div className="hidden md:flex h-20 items-center justify-between px-5 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-30">
          <div className="flex items-center gap-4 bg-slate-100/80 dark:bg-slate-950 border border-slate-200/50 dark:border-slate-800/50 px-5 py-2.5 rounded-2xl w-[520px] group transition-all focus-within:ring-4 focus-within:ring-blue-500/10 focus-within:bg-white dark:focus-within:bg-slate-900 focus-within:border-blue-500/30">
            <Search size={18} className="text-slate-400 group-focus-within:text-blue-500 transition-colors" />
            <input 
              type="text" 
              placeholder="Query database records..." 
              className="bg-transparent border-none text-[13px] font-bold focus:ring-0 w-full outline-none placeholder:text-slate-400 placeholder:uppercase placeholder:tracking-widest"
            />
          ...
          <div className="flex items-center gap-6">
            <div className="hidden lg:flex flex-col items-end">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Node Status: Active</span>
              </div>
              <span className="text-xs font-mono font-bold text-blue-600 dark:text-blue-400 mt-0.5">MM_STABLE_V3.1.4</span>
            </div>
