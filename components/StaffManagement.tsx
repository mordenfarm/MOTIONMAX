        ...
        multiple && availableClasses ? (
          <div className="flex flex-wrap gap-2 py-2">
            {availableClasses.map(cls => (
              <label key={cls} className="flex items-center gap-2 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 px-4 py-1.5 rounded-xl cursor-pointer hover:border-blue-500 transition-all select-none">
                <input 
                  type="checkbox"
    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-[1400px] mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          {isAdmin && (
            <button onClick={() => { setIsAdding(true); setSelectedClassesForAdd([]); setAddFormNationality('Zimbabwean'); }} className="flex items-center gap-3 px-8 py-3.5 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:bg-black transition-all active:scale-95">
      ...
      <div className="bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-sm">
        {viewMode === 'table' ? (
                ...
                {filteredStaff.length === 0 ? (
                  ...
                  <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-blue-900/10 cursor-pointer group transition-colors" onClick={() => { setSelectedStaff(s); setIsEditing(false); setEditForm(s); }}>
                    ...
                    <td className="px-8 py-6">
                      <span className="px-4 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase border border-blue-100 dark:border-blue-800 shadow-sm">{s.position}</span>
                    </td>
          ...
          <div className="p-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredStaff.map(s => (
              <div key={s.id} className="bg-slate-50 dark:bg-slate-950 border-2 border-transparent hover:border-blue-500 dark:bg-slate-800 rounded-[2rem] p-8 hover:shadow-2xl transition-all cursor-pointer group relative overflow-hidden" onClick={() => { setSelectedStaff(s); setIsEditing(false); setEditForm(s); }}>
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-100 transition-opacity">
                   <ShieldCheck size={40} className="text-blue-600" />
                </div>
                <div className="w-16 h-16 rounded-2xl bg-slate-900 text-white flex items-center justify-center font-black text-2xl mb-6 uppercase shadow-lg group-hover:scale-110 transition-transform">{s.fullName[0]}</div>
      ...
      {selectedStaff && (
        <div className="fixed inset-0 z-[200] flex justify-end">
          <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-md" onClick={() => setSelectedStaff(null)} />
          <aside className="relative w-full max-w-2xl bg-white dark:bg-slate-950 shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 border-l border-slate-100 dark:border-slate-800">
            <header className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md z-20 shadow-sm">
              <div className="flex items-center gap-6">
                <div className="w-16 h-16 rounded-[1.5rem] bg-slate-900 text-white flex items-center justify-center font-black text-2xl uppercase shadow-xl">{selectedStaff.fullName[0]}</div>
                <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setSelectedStaff(null)} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-400 hover:text-black transition-colors"><X size={24}/></button>
              </div>
            </header>
               ...
               <div className="space-y-8">
                  ...
                  <table className="w-full">
                     <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                        ...
                        <ProfileRow label="Assigned Classes" value={selectedStaff.assignedClasses} field="assignedClasses" isEditing={isEditing} editForm={editForm} setEditForm={setEditForm} multiple availableClasses={settings?.classes || []} />
                        ...
                        {selectedStaff.nationality !== 'Zimbabwean' && (
                          <ProfileRow label="Passport Number" value={selectedStaff.passportNumber} field="passportNumber" isEditing={isEditing} editForm={editForm} setEditForm={setEditForm} />
                        )}
            ...
            {showDeleteConfirm && (
              <div className="absolute inset-0 z-50 bg-white/95 dark:bg-slate-950/95 flex flex-col items-center justify-center p-12 text-center animate-in zoom-in-95 backdrop-blur-md">
                 ...
                 <h3 className="text-3xl font-black uppercase text-slate-900 dark:text-white tracking-tight">Delete Staff Member?</h3>
                 <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mt-4 max-w-sm mx-auto leading-relaxed">This will permanently remove this employee and disable their account. This cannot be undone.</p>
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 shadow-2xl rounded-[3rem] overflow-hidden animate-in zoom-in-95 max-h-[90vh] overflow-y-auto sidebar-scrollbar border border-slate-200 dark:border-slate-800">
             ...
             <form onSubmit={async (e) => {
               ...
               try {
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-4 py-1.5 rounded-full border border-blue-100 dark:border-blue-800">01 Identification</span>
                    <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800"></div>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input required name="firstName" className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white shadow-inner" placeholder="Enter first name..." />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-950 dark:text-slate-400 ml-2">Surname</label>
                      <input required name="lastName" className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white shadow-inner" placeholder="Enter surname..." />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <select 
                          ...
                          className="w-full pl-16 pr-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white appearance-none cursor-pointer shadow-inner"
                        >
                    ...
                    {addFormNationality !== 'Zimbabwean' ? (
                      <div className="space-y-2 animate-in slide-in-from-left duration-300">
                        ...
                        <div className="relative">
                          ...
                          <input required name="passportNumber" placeholder="Enter passport ID" className="w-full pl-16 pr-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white shadow-inner" />
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-950 dark:text-slate-400 ml-2">National ID</label>
                        <input name="nationalId" placeholder="63-XXXXXX-X-XX" className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white shadow-inner" />
                      </div>
                    )}
                  </div>
                </section>
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-4 py-1.5 rounded-full border border-blue-100 dark:border-blue-800">02 Contact Info</span>
                    <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800"></div>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input required type="email" name="email" className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white shadow-inner" placeholder="name@motionmax.co.zw" />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input required type="tel" name="phone" placeholder="+263..." className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white shadow-inner" />
                    </div>
                  </div>
                </section>
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-4 py-1.5 rounded-full border border-blue-100 dark:border-blue-800">03 Job Details</span>
                    <div className="h-px flex-1 bg-slate-100 dark:bg-slate-800"></div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <select name="position" className="w-full px-6 py-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border-2 border-transparent focus:border-blue-600 outline-none transition-all font-bold dark:text-white appearance-none cursor-pointer shadow-inner">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                      ...
                      <span className="text-[9px] font-black uppercase text-blue-600 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-lg border border-blue-100 dark:border-blue-800">
                      ...
                      {(settings?.classes || []).map(cls => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-tight transition-all border-2 ${
                              isSelected 
                ...
                <button 
                  ...
                  className="w-full py-6 bg-slate-900 dark:bg-blue-600 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] text-[11px] shadow-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-50"
                >
